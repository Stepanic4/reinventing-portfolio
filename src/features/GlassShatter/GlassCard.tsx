...
export const GlassCard: React.FC<GlassCardProps> = ({ imageSrc, title, description, link }) => {
    ...
    useEffect(() => {
        const scripts = [
            ...
            { src: "https://s3-us-west-2.amazonaws.com/s.cdpn.io/175711/delaunay.js", id: 'delaunay-script' }
        ];
        scripts.forEach(s => {
        ...
        <>
            ...
            {showModal && (
                <div className="fixed inset-0 z-[999] flex items-center justify-center p-4 md:p-10">
                    <div className="absolute inset-0 bg-white/20 dark:bg-black/40 backdrop-blur-xl animate-in fade-in duration-500" onClick={closeModal}/>
                    <div className="relative w-full max-w-5xl bg-white dark:bg-[#020617] rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col md:flex-row border border-black/5 dark:border-white/10 animate-in zoom-in-95 duration-500">
                        <button onClick={closeModal} className="absolute top-6 right-6 w-10 h-10 flex items-center justify-center rounded-full bg-black/5 hover:bg-black/10 dark:bg-white/10 dark:hover:bg-white/20 transition-all z-50">
                        ...
                        <div className="w-full md:w-1/2 p-8 md:p-12 flex flex-col justify-center">
                            <h2 className="text-3xl md:text-5xl font-black uppercase italic dark:text-white mb-6 tracking-tighter">{title}</h2>
                            <p className="text-slate-600 dark:text-slate-400 text-lg mb-10 leading-relaxed">
                                {description || "Описание проекта."}
                            </p>
                            {link && (
                                <a href={link} target="_blank" rel="noopener noreferrer" className="inline-block px-10 py-4 bg-black dark:bg-white text-white dark:text-black font-bold rounded-2xl text-center transition-transform hover:scale-105 w-fit">
